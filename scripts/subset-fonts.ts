...
/**
 * 扫描 dist/ 中所有 CSS/HTML/JS，找到 Astro Font API 为指定 family 生成的 @font-face，
 * 返回其 src url 列表（指向 /_astro/fonts/*.woff2）。
 * Astro 生成的 family 名会带 hash 后缀（如 Chikushi-A-maru-6b156cd0eb60f454），故正则可选匹配。
 */
function escapeRegExp(s: string): string {
	return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

async function findAstroFontUrls(family: string): Promise<string[]> {
	const files = await glob(`${DIST_DIR}/**/*.{css,html,js}`);
	const re = new RegExp(
		`font-family:\\s*["']?${escapeRegExp(family)}(?:-[a-f0-9]+)?["']?[^}]*?url\\(\\s*["']?([^"')]+?)["']?\\s*\\)`,
		"gs",
	);
...
async function main() {
	...
	for (const font of localSubsetFonts) {
		...
		try {
			...
			results.push({
				...
				// 定位 Astro 实际生成的字体引用 url（dist 中 _astro/fonts/*.woff2）
				originalDistUrls: await findAstroFontUrls(font.family),
			});
		} catch (err) {
	...
	// 5. 替换 dist/ 中 CSS/HTML/JS 里 Astro Font API 实际生成的字体引用
	//    Astro 生成的 @font-face 使用自身 hash URL（如 /_astro/fonts/a5fdc1b...woff2），
	//    并非 __SUBSET_FONT_xxx__ 占位符，因此按 family 名匹配真实 src url 进行替换。
	console.log("🔄 Replacing font URLs in dist/ (CSS/HTML/JS)...");
	const filesToReplace = await glob(`${DIST_DIR}/**/*.{css,html,js}`);
	...
	for (const file of filesToReplace) {
		...
		for (const result of results) {
			...
			for (const oldUrl of result.originalDistUrls) {
				if (content.includes(oldUrl)) {
					content = content.split(oldUrl).join(subsetUrl);
					replaced = true;
				}
			}
			// 兼容旧占位符机制（如有）
			...
			if (content.includes(placeholder)) {
				content = content.split(placeholder).join(subsetUrl);
				replaced = true;
			}
		}
	...
	// 6. 清理 dist/ 中的原始字体文件（Astro 输出到 _astro/fonts/*.woff2 的那份）
	console.log("🗑 Cleaning up original font files from dist/...");
	for (const result of results) {
		if (result.originalDistUrls.length === 0) {
			console.warn(
				`   ⚠ No Astro font reference found for '${result.family}', original file not removed.`,
			);
			continue;
		}
		for (const oldUrl of result.originalDistUrls) {
			const originalInDist = path.join(DIST_DIR, oldUrl);
			try {
